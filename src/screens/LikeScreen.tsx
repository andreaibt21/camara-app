...
const NiceListScreen = () => {
  ...
  const getBlob = async (image: any) => {
    return await new Promise((resolve, reject) => {
      ...
      xhr.onload = function () {
        ...
        console.log("dentro de get blob ", xhr);
      };
      xhr.onerror = function (e) {
        console.log("on error dentro de getblob ", e);
  ...
  const uploadImage = async (image: any, type: string) => {
    ...
    const fileName = image.substring(image.m + 1);
  ...
  const handleCamera = async () => {
    ...
    if (!result.cancelled) {
      await uploadImage(result["uri"], "nice")
        .then(() => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .catch((ex) => console.error("use effect ", ex.stringify()));
      ...
      headerRight: () => (
        <View style={styles.buttonAccessCamera}>
          <TouchableOpacity style={styles.buttonCamera} onPress={handleCamera}>
            <FontAwesome name="camera" size={24} color="#ffc400" />
          </TouchableOpacity>

          <TouchableOpacity onPress={handlerSingOut}>
            <FontAwesome name="power-off" size={24} color="#ffc400" />
          </TouchableOpacity>
        </View>
      ),
      headerLeft: () => (
        <View style={styles.buttonAccessCamera}>
          <TouchableOpacity onPress={handlerBack}>
            <FontAwesome name="step-backward" size={24} color="#ffc400" />
          </TouchableOpacity>
  ...
  const getDocuments = async () => {
    ...
    try {
      ...
      console.log("dentro de getdocuments ", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleVote = async (id: string) => {
    try {
      ...
      console.log("dentro de handleVote ", error);
    } finally {
      setLoading(false);
    }
  };
